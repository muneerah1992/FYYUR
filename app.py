...
class Venue(db.Model):
    ...
    shows = db.relationship('Shows', cascade="delete, delete-orphan", backref='Venue', lazy=True)
...
def venues():
  ...
  data = []    
  for area in areas:
    venue_query = Venue.query.filter_by(state=area.state).filter_by(city=area.city).all()
    venue_data = []
...
def search_venues():
  ...
  search_result = db.session.query(Venue).filter(Venue.name.ilike(f'%{search_term}%')).order_by('name').all()
  data = []
...
def show_venue(venue_id):
  ...
  fixed_genres = ""
  genres1 = venue.genres
  for ch in genres1:  
        fixed_genres += ch   

  fixed_genres = fixed_genres[1:len(fixed_genres) - 1] + " ";
  fixed_genres2 = fixed_genres.split(',')
...
def create_venue_submission():
  try:
    ...
    venue = Venue(
      ...
      seeking_description= form.seeking_description.data, website= form.website.data 
      )
...
def artists():
  ...
  data = db.session.query(Artist).order_by('name').all()
...
def search_artists():
  search_term = request.form.get('search_term', '')
  search_result = db.session.query(Artist).filter(Artist.name.ilike(f'%{search_term}%')).order_by('name').all()
  data = []
...
def create_artist_submission():
  ...
  try:
    ...
    artist = Artist(
      ...
      phone=form.phone.data, genres=form.genres.data, facebook_link=form.facebook_link.data, 
      image_link=form.image_link.data, seeking_venue= form.seeking_venue.data, 
      seeking_description= form.seeking_description.data, website= form.website.data 
      )
    # commit session to database
...
def create_show_submission():
  try:
    form = ShowForm()
    show = Shows(
      venue_id=form.venue_id.data, artist_id=form.artist_id.data, start_time=form.start_time.data
      )
    # commit session to database
    db.session.add(show)
    db.session.commit()
     # on successful db insert, flash success
    flash('Shwo was successfully listed!')
  except:
